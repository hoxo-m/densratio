...
![](reference/figures/README-compute-estimated-density-ratio-1.png)
...
![](reference/figures/README-compare-true-estimate-1.png)
...
![](reference/figures/README-basics-compute-estimated-density-ratio-1.png)

### 3.2 Methods

[`densratio()`](http://hoxo-m.github.io/densratio/reference/densratio.md)
has `method` argument that you can pass `"uLSIF"`, `"RuLSIF"`, or
`"KLIEP"`.
...
- Optimizes the kernel weights `theta` (in other words, find the optimal
  coefficients of the linear model), and
- The parameters `sigma` and `theta` are saved into `densratio` object,
  and are used when to compute density ratio in the call
  `compute_density_ratio()`.
...
![](reference/figures/README-compare-2d-1.png)
